...
// ============================================================
// TIDSDETEKTIVERNA — kartbaserad prototyp v2
// Kartan är menyn. Karaktärerna är värdar för varje uppdrag.
//
// VIKTIGT: Lägg dessa filer i din public/-mapp:
//   map.jpg
//   tickelton.jpg
//   mira.jpg
//   klonk.jpg
...
// Varje plats har också sin karaktär som värd.
const HOTSPOTS = {
  reading: {
    key: "reading",
    title: "Bokgränden",
...
export default function App() {
  ...
  function openMission(key) {
    if (key === "timemachine") {
      if (allDone) setCurrentMission("end");
      return;
    }
    setFeedback(null);
  ...
  const activeHotspot = currentMission && HOTSPOTS[currentMission];
  ...
  return (
    <div className="td-app">
      ...
      {currentMission === "reading" && (
        <MissionOverlay hotspot={activeHotspot} onClose={closeMission}>
          <ReadingMission
            feedback={feedback}
            setFeedback={setFeedback}
            alreadyDone={completed.reading}
            onComplete={() => complete("reading")}
            onBack={closeMission}
          />
        </MissionOverlay>
      )}
      {currentMission === "clock" && (
        <MissionOverlay hotspot={activeHotspot} onClose={closeMission}>
          <ClockMission
            feedback={feedback}
            setFeedback={setFeedback}
            alreadyDone={completed.clock}
            onComplete={() => complete("clock")}
            onBack={closeMission}
          />
        </MissionOverlay>
      )}
      {currentMission === "puzzle" && (
        <MissionOverlay hotspot={activeHotspot} onClose={closeMission}>
          <PuzzleMission
            feedback={feedback}
            setFeedback={setFeedback}
            alreadyDone={completed.puzzle}
            onComplete={() => complete("puzzle")}
            onBack={closeMission}
          />
        </MissionOverlay>
      )}
      {currentMission === "end" && (
        <MissionOverlay grand onClose={closeMission}>
...
// ============================================================
// STARTSKÄRM med alla tre karaktärer
// ============================================================
function StartScreen({ onStart }) {
  return (
    <div className="td-start-screen">
      ...
      <div className="td-map" style={{ backgroundImage: `url(${ASSETS.map})` }}>
        ...
        {visibleHotspots.map((key) => {
          ...
          return (
            <button
              key={key}
              className={`td-hotspot ${done ? "td-hotspot-done" : ""} ${
                isHovered ? "td-hotspot-hover" : ""
              }`}
              style={{
                left: `${h.x}%`,
                top: `${h.y}%`,
        ...
        <button
          className={`td-hotspot td-hotspot-finale ${
            allDone ? "td-hotspot-finale-active" : "td-hotspot-finale-locked"
          } ${hovered === "timemachine" ? "td-hotspot-hover" : ""}`}
          style={{
          ...
          onClick={() => openMission("timemachine")}
  ...
  return (
    ...
    >
      ...
      <div className="td-hover-content">
        ...
        <div
          className={`td-hover-status ${
            done || (isFinale && allDone) ? "ok" : ""
          }`}
        >
          {status}
        </div>
...
// ============================================================
// Overlay för uppdrag — visar karaktären som värd
// ============================================================
...
function ReadingMission({ feedback, setFeedback, alreadyDone, onComplete, onBack }) {
  ...
  function pick(c) {
    if (c.correct) {
      ...
      setFeedback({
        type: "error",
        text: "Nästan! Ledtråd: titta efter orden röd, väska och katt.",
      });
    }
  }
  return (
    <>
      <p className="td-mission-text">
        <em>"Hjälp mig hitta vad detektiven tappat!"</em> Mira håller upp ledtråden:
        Hitta den <strong>röda väskan</strong> bredvid <strong>katten</strong>.
      </p>
...
function ClockMission({ feedback, setFeedback, alreadyDone, onComplete, onBack }) {
  ...
  function pick(c) {
    if (c.correct) {
      ...
      setFeedback({
        type: "error",
        text: "Inte riktigt. Ledtråd: halv tre betyder 30 minuter innan tre.",
      });
    }
  }
  return (
  ...
  return (
    <style>{`
      ...
      .td-hotspot-finale-active {
        background: rgba(253, 201, 77, 0.3);
        border: 3px solid var(--ink);
      ...
      .td-overlay-host-img img {
        width: 100%;
        height: 100%;
        object-fit: cover;
      }
      .td-overlay-host-name {
        font-size: 13px;
        font-style: italic;
        opacity: 0.75;
        margin-top: 2px;
